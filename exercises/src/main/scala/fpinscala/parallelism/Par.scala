...
object Par {
  ...
  def sequence[A](xs: List[Par[A]]): Par[List[A]] = {
    xs.headOption match {
      case None => unit(Nil)
      case Some(x) => map2(x, sequence(xs.tail))((y, ys) => y :: ys)
    }
  }

  def parMap[A, B](ps: List[A])(f: A => B): Par[List[B]] = sequence(ps.map(a => asyncF(f)(a)))

  def parFilter[A, B](ps: List[A])(f: A => Boolean): Par[List[A]] = {
    val pars: List[Par[List[A]]] = ps map asyncF(a => if (f(a)) List(a) else Nil)
